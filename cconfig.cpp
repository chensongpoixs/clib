...
namespace chen {
	...
	void cconfig::set_int16(int32 index, const std::string key, int16 value)
	{
		if (m_values_size < index)
		{
	...
	void cconfig::set_uint16(int32 index, const std::string key, uint16 value)
	{
		if (m_values_size < index)
		{
    ...
    int16 cconfig::get_int16(int64 index)const
    {
        if (m_values_size < index)
        {
            ERROR_EX_LOG("get_int16 index = %u, num = %u", m_values_size, index);
            return 0;
        }
        return m_configs[index].m_int16;
    }
    uint16 cconfig::get_uint16(int64 index)const
    {
        if (m_values_size < index)
        {
            ERROR_EX_LOG("get_int32 index = %ld, num = %ld", m_values_size, index);
        ...
        return m_configs[index].m_uint16;
    }

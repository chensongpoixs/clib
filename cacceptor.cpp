...
//		// nlonl
    ...
    bool cacceptor::set_nosigpipe(socket_type & sockettype)
    {
        int set = 1, ret = 1;
#if defined(SO_NOSIGPIPE)
        ret= setsockopt(sockettype, SOL_SOCKET, SO_NOSIGPIPE, (char*)&set, sizeof(int));
        if (ret == -1)
        {
            WARNING_LOG("set  SO_NOSIGPIPE failed !!!");
            return false;
        }
#endif
        return true;
    ...
    bool            cacceptor::sync_wait()
	{
		...
		if (::poll(&pfd, 1, 1000) == 1) {
			...
			if ((pfd.revents & POLLOUT) || (pfd.revents & POLLHUP) || (pfd.revents & POLLERR))
			{
				return true;
			}
	...
	bool cacceptor::set_nonagle(bool on)
	{
		...
		if (ret == SOCKET_ERROR)
		{
			ERROR_EX_LOG(" set no nagle failed !");
			return false;
		}
		return true;
	}

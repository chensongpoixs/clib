...
#error unexpected c complier (msc/gcc), Need to implement this method for demangle
		...
		gen_log_file_name(log_name, path + "/", name, ext, ELogName_DateTime, m_date_time);
...
*/
	...
	void casync_log::append_var(ELogLevelType level, const char * format, va_list ap)
	{
		...
		//static const unsigned int max_size = 1024;
		{
			...
			cnt = vsnprintf(m_log_buf, LOG_BUF_MAX_SIZE, format, ap);
	...
	void casync_log::_work_pthread()
	{
		...
		while (!m_stoped)
		{
			...
			/*static std::ostringstream  input;
			input.clear();*/
...
//					std::lock_guard<std::mutex> lock{ m_lock };
				...
				if (log_item_ptr->buf)
				{
                    ...
                    log_item_ptr->buf = NULL;
				}

				delete log_item_ptr;
                log_item_ptr = NULL;
			}
			//没有日志文件要写入时， 在同步屏蔽上和文件中
			if (m_log_item.size() <= 0)
			{
				if (m_fd.is_open())
				{
	...
	void casync_log::_handler_log_item(const clog_item * log_item_ptr)
	{
		...
		if (!(log_item_ptr->level == ELogLevel_Info || log_item_ptr->level == ELogLevel_System  || log_item_ptr->level == m_level_log))
		{
			...
			//log_buf_input << '[' << dateTime << "][" << g_log_name_colors[log_item_ptr->level].name << "]";
			if (m_fd.is_open())
			{
		...
		//性能瓶颈  日志IO上---> flush 上有性能
	//	log_buf_input.write(log_item_ptr->buf, log_item_ptr->size);
	//	m_fd.write("\n", 1);
		if (m_fd.is_open() && log_item_ptr->size > 0)
		{
			m_fd.write(log_item_ptr->buf, log_item_ptr->size);
			m_fd.write("\n", 1);
		//	m_fd.flush();
		}
		if (m_show_screen && log_item_ptr->size > 0)
		{

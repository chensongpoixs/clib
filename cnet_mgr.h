...
namespace chen {
	...
	public:
		...
		//新消息回调
		typedef std::function<void(cnet_msg &msg_ptr)>								cmsg_cb;
	private:
	...
	public:
		...
		* @param max_session : 作为服务端 是转入大于0
	...
	public:
		...
		**/
		bool 		send_msg(uint32 sessionId, uint16 msg_id, const void* msg_ptr, int64 msg_size);

		// 指定服务器发送
		bool 		send_msg(uint32 sessionId, uint16 msg_id, const void* msg_ptr, int64 msg_size, int32 extra);
		/**
	...
	private: 	
        ...
        clock_type                      m_send_data_mutex;
		bool							m_listening; //服务器 三次 ack的
	};
} // chen 

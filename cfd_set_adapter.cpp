...
namespace chen {
	...
	{
		m_fd_set = static_cast<cfd_set*>(::operator new(
			sizeof(cfd_set) - sizeof(socket_type)
	...
	void cfd_set_adapter::remove(socket_type descriptor)
	{
		for (uint32 i = 0; i < m_fd_set->m_count; ++i)
		{
			if (m_fd_set->m_fd_array[i] == descriptor)
			{
				for (uint32 j = i; j < m_fd_set->m_count; ++j)
				{
					m_fd_set->m_fd_array[j] = m_fd_set->m_fd_array[j + 1];
				}
				--m_fd_set->m_count;
				return;
			}
	...
	void cfd_set_adapter::reserve(uint32 size)
	{
		...
		cfd_set* new_fd_set = static_cast<cfd_set*>(::operator new(
			sizeof(cfd_set) - sizeof(socket_type)
